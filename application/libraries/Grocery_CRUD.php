...
/* Para obtener solo el array de datos devueltos .. by esteban chamba */        
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected $js_config_files			= array();
        ...
        /* Agregadas por esteban chamba*/
        public $conf_insert_url = null;
        public function set_conf_insert_url($insert_url){
            $this->conf_insert_url = $insert_url;
        }
        public function get_conf_insert_url(){
            return $this->conf_insert_url;
        }
        /* Agregadas por esteban chamba - para cambiar la vista del add new */
        public $add_file = 'add.php';        
        public function set_add_file($file){
            $this->add_file = $file;
        }
        public function get_add_file(){
...
//
		...
		foreach($list as $num_row => $row)
		{
			foreach($output_columns as $column)
			{
				...
				if( $has_callbacks && isset($this->callback_column[$field_name]) )
					$list[$num_row]->$field_name = call_user_func($this->callback_column[$field_name], $field_value, $row, $field_name); /* by esteban chamba .. agregado , $field_name para pasar como parametro tambien el nombre de la columna, 08 oct. 2014*/
	...
	protected function showAddForm()
	{
		...
		$data->list_url 		= $this->getListUrl();
                ...
                /* 
                 * By esteban chamba 26 sept. 2014
                 * esto es para cambiar el action del form que inserta los datos
                 */                
                $config_insert_url = $this->get_conf_insert_url();
                if(!empty($config_insert_url)){
                    $data->insert_url		= $this->get_conf_insert_url();    
                }else{
...
//		$data->insert_url		= $this->getInsertUrl();
		...
		$this->_theme_view($this->get_add_file(),$data);
	...
	protected function update_layout($update_result = false, $state_info = null)
	{
		...
		{
//                    echo tagcontent('script', "alertaExito('".$this->l('update_success_message')."')");
...
//	protected $default_theme		= 'flexigrid';
	protected $default_theme		= 'datatables2';
	...
	/**
	 *
	 * By Esteban Chamba, The fields that user can see . It is only for the add form
         * $fields => un array con los campos q se muestran al agregar datos a la tabla del crud
	 */
	public function add_fields2($fields)
	{
	 ...
	 * By Esteban Chamba, The fields that user can see . It is only for the add form
         * $fields => un array con los campos q se muestran al agregar datos a la tabla del crud
	 */
	public function edit_fields2($fields)
	{
